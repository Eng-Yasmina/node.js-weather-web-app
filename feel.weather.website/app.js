...
const new_York= document.getElementById('New-York');
const los_Angeles = document.getElementById('Los-Angeles');
const mountain_View = document.getElementById('Mountain-View');
const goBackBtn = document.getElementById('goBack');

/**
 * End Global Variables
 * Starts Helper Functions
 * IMPORTANT: in my app.js file, helper fns means the generation fns which hold zip codes
 */

/**
 * @description creates generation fn that hold the zip code entered by the user.
 */
const generateAction = () => {
    // newZip and userFeeling are local variables because i want to store them just after the user hit generate
    newZip = document.getElementById('zip').value;
    main();
}

/**
 * @description creates ready generation fns for New York, Los Angeles, and Mountain View cities with built-in zip codes.
 */
const newYork = () => {
    // newZip and userFeeling are local variables because i want to store them just after the user hit generate
    newZip = 10001;
...
const main = () => {
    ...
    getWeather(baseUrl,newZip,apiKey).then((recievedData) =>{
        console.log(recievedData);
        //I noticed that when i pass a wrong zip code, it returns (recievedData) object with prop (.cod = 404), in the console. So i used it as a condition to resolve or reject to excute the task.
        if (recievedData.cod != 404) {
            ...
            postData('/addData',{newDate, temprature, city, description, userFeeling, TimeZone, newZip});
            toggleinput();
            retrieveData();
        } else {
            // desplays error msg if the user entered an invalid zip code
 ...
 */
const getWeather = async (baseurl, userzipcode, apikey) => {
    // [fetch call]: Get request using fetch to the weather web API
    const request = await fetch(`${baseurl}${userzipcode}&appid=${apikey}`);
    try{
        // transform into JSON
 ...
 */
//Request from the client side code to execute the POST route
...
//Get request from the client side code to retrieve the stored data from the server.
/**
...
const retrieveData = async () => {
    ...
    try {
        ...
        document.getElementById('temp').innerHTML = `${Math.round(allData.temp)}<img style="transform: translateY(-150px)" src="https://img.icons8.com/external-royyan-wijaya-detailed-outline-royyan-wijaya/48/f0f0f0/external-fahrenheit-weather-royyan-wijaya-detailed-outline-royyan-wijaya.png"/>`;
...
const toggleinput = () => {
    ...
    //add option to go back to the input box for a new zip code
...
// TODO: add event listener (generateAction) when clicking on generate button
 ...
 // TODO: run function newYork when the user choose New York city
 new_York.addEventListener('click', newYork);

 // TODO: show the temp, local time, date when the user choose to query Los Angeles city
 los_Angeles.addEventListener('click', losAngeles);

 // TODO: run event listener (mountainView) when the user choose Mountain View city
 mountain_View.addEventListener('click', mountainView);
